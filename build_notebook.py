...
def code(idx, source_lines):
    src = source_lines if isinstance(source_lines, list) else [source_lines]
    joined = "".join(src)
    try:
        ast.parse(joined)
        print(f"cell[{idx:02d}] syntax OK")
    except SyntaxError as e:
        print(f"cell[{idx:02d}] SYNTAX ERROR: {e}", file=sys.stderr)
        sys.exit(1)
    return {
        "cell_type": "code",
...
# ── Cell 02: ## 1. 特征分析 markdown ──────────────────────────────────────────
cells.append(md("## 1. 特征分析"))
print("cell[02] md OK")
...
# ── Cell 04: ## 2. WOE/IV 分箱 markdown ───────────────────────────────────────
cells.append(md("## 2. WOE/IV 分箱"))
print("cell[04] md OK")
...
# ── Cell 06: ## 3. 特征筛选 markdown ──────────────────────────────────────────
cells.append(md("## 3. 特征筛选"))
print("cell[06] md OK")

# ── Cell 07: 特征筛选 + ReportGenerator + feature_selection_report ─────────────
cells.append(code(7, [
...
# ── Cell 08: ## 4. 模型训练（LightGBM 分类）markdown ──────────────────────────
cells.append(md("## 4. 模型训练（LightGBM 分类）"))
print("cell[08] md OK")
...
# ── Cell 11: ## 5. 自定义超参数空间（可选）markdown ───────────────────────────
cells.append(md("## 5. 自定义超参数搜索空间（可选）"))
print("cell[11] md OK")
...
# ── Cell 14: ## 6. 模型评估报告 markdown ──────────────────────────────────────
cells.append(md("## 6. 模型评估报告"))
print("cell[14] md OK")

# ── Cell 15: 评估报告 code ─────────────────────────────────────────────────────
cells.append(code(15, [
    "# 特征重要性（从训练好的 LGBM Booster 取）\n",
    "fi_df = pd.DataFrame({\n",
...
# ── Cell 17: ## 7. 特征分析报告 markdown ──────────────────────────────────────
cells.append(md("## 7. 特征分析报告"))
print("cell[17] md OK")
...
with open("demo_lgbm_clf.ipynb", "r", encoding="utf-8") as f:
    orig = json.load(f)

nb = {
    "nbformat": orig["nbformat"],
    "nbformat_minor": orig.get("nbformat_minor", 5),
...
with open("demo_lgbm_clf.ipynb", "w", encoding="utf-8") as f:
